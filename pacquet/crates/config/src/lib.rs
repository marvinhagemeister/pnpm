...
impl Config {
        ...
        Sys: EnvVar + EnvVarOs + GetHomeDir + LinkProbe,
    {
        ...
        let mut settings_root_dir = start_dir.to_path_buf();
        if let Some((base_dir, settings)) = workspace_yaml {
            settings_root_dir = base_dir.clone();
        ...
        // Legacy root manifest settings (`package.json#pnpm`) are still
        // accepted by pnpm and can appear in pnpm-generated lockfiles.
        // Apply the subset pacquet currently understands before env vars
        // so `PNPM_CONFIG_*` continues to be the highest-precedence config
        // file overlay.
        if let Some(mut settings) = read_legacy_package_pnpm_settings(&settings_root_dir) {
            settings.substitute_env::<Sys>();
            settings.apply_to(&mut self, &settings_root_dir);
        }
...
fn read_legacy_package_pnpm_settings(dir: &std::path::Path) -> Option<WorkspaceSettings> {
    let text = fs::read_to_string(dir.join("package.json")).ok()?;
    let value: serde_json::Value = serde_json::from_str(&text).ok()?;
    let pnpm = value.get("pnpm")?.as_object()?;
